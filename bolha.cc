...
void bolha(LISTA lista[])
{
	for(int i = (quantidade-1); i > 0; i--)
	{
		for(int j = 0; j < i; j++)
		{
			if( strcmp(getAnoNascimento(lista[j].array), getAnoNascimento(lista[j+1].array)) > 0  )
			{
				PERSONAGEM tmp = newPersonagem();
				tmp = clone(lista[j].array);
				lista[j].array = clone(lista[j+1].array);
				lista[j+1].array = clone(tmp);
			}
		}
	}
}
