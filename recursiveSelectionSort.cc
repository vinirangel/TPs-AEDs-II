...
/*void selectionRec(LISTA lista[],int indice,int i, int j)
  {
  if(i < quantidade-1)
  {
  indice = i;
  ...
  selectionRec(lista,indice,i++,j);
  }
  }

  void selectionSort(LISTA lista[])
  {
  selectionRec(lista,0,0,0);
  }*/

void selection(LISTA lista[], int i, int j, int size, int flag)
{
	PERSONAGEM temp;
	if (i < size - 1)
	{
		if (flag)
		{
			printf("ENTREI NO FLAG\n");
			j = i + 1;
		}
		if (j < size)
		{
			printf("VALOR DO J E = %d\n",j);
			printf("VALOR DO SIZE E = %d\n",size);
			if (strcmp( getNome(lista[i].array), getNome(lista[j].array)) > 0)
			{
				printf("ORDENANDO\n");
				temp = clone(lista[i].array);
				lista[i].array = clone(lista[j].array);
				lista[j].array = clone(temp);
			}
			printf("RECURSIVIDADE INTERIOR\n");
			selection(lista, i, j + 1, size, 0);
		}
			printf("RECURSIVIDADE DE FORA\n");
		selection(lista, i + 1, 0, size, 1);
	}
}
...
int main(int argc, char** argv)
{
	...
	selection(lista, 0, 0, quantidade, 1);
	mostrarArray(lista);
}
