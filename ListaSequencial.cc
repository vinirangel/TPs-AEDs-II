...
int main(int argc, char** argv)
{
	...
	{
		if(strstr(entrada2[x],"II") != NULL)
		{
			nome = strstr(entrada2[x],"/");
			personagem[x] = newPersonagem();
			personagem[x] = lerArquivo(nome,personagem[x]);
			personagem[x] = setNome(personagem[x]);
			inserirInicio(lista,personagem[x]);
		}
		if(strstr(entrada2[x],"I*") != NULL)
		{
			nome = strstr(entrada2[x],"/");
			char* tmp = strtok(entrada2[x]," ");
			tmp = strtok(NULL," ");
			int pos = atoi(tmp);
			personagem[x] = newPersonagem();
			personagem[x] = lerArquivo(nome,personagem[x]);
			personagem[x] = setNome(personagem[x]);
			inserir(lista,personagem[x],pos);
		}
		if(strstr(entrada2[x],"IF") != NULL)
		{
			nome = strstr(entrada2[x],"/");
			personagem[x] = newPersonagem();
			personagem[x] = lerArquivo(nome,personagem[x]);
			personagem[x] = setNome(personagem[x]);
			inserirFim(lista,personagem[x]);
		}
		if(strstr(entrada2[x],"RI") != NULL)
		{
			printf("(R) %s\n",getNome(removerInicio(lista)));
		...
		if(strstr(entrada2[x],"R*") != NULL)
		{
			...
			tmp = strtok(NULL," ");
			int pos = atoi(tmp);
			printf("(R) %s\n",getNome(remover(lista,pos)));
		}
		if(strstr(entrada2[x],"RF") != NULL)
		{
			printf("(R) %s\n",getNome(removerFim(lista)));
