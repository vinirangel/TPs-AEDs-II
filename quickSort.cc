...
void quicksortRec (LISTA lista[], int esq, int dir) {
	int i = esq, j = dir;
	char* pivo = getCorDoCabelo(lista[(dir+esq)/2].array);
	while (i <= j) {
		while (strcmp( getCorDoCabelo(lista[i].array), pivo) < 0) i++;
		comp++;
		while (strcmp( getCorDoCabelo(lista[j].array), pivo) > 0) j--;
		comp++;
		if (i <= j) {
			PERSONAGEM tmp = clone(lista[j].array);
			mov++;
			lista[j].array = clone(lista[i].array);
			mov++;
...
void quicksort(LISTA lista[]) {
	quicksortRec(lista, 0, quantidade-1);
}
	...
	for (int i = 0; i < (quantidade - 1); i++) {
		...
		for (int j = (i + 1); j < quantidade; j++){
			if(strcmp( getNome(lista[indice].array), getNome(lista[j].array)) > 0 
					&& strcmp( getCorDoCabelo(lista[i].array), getCorDoCabelo(lista[j].array) ) == 0){
				indice = j;
				comp++;
			}
		}
