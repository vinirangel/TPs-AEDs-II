...
public class heapSort{
	...
	public void construirHeap(String[] linha,int numEntrada)
	{
	...
	{
		for(int i = tamHeap; i > 1 && array[i].getAltura().compareTo(array[i/2].getAltura) > 0; i /= 2)
		{
			Personagem tmp = new Personagem();
			tmp = tmp.clone(array[i]);
			array[i/2] = array[i/2].clone(array[i]);
	...
	public static void reconstroi(int tamHeap)
	{
		...
		while(i <= (tamHeap/2))
		{
			if(array[2*i].getAltura().compareTo(array[2*i+1].getAltura() || 2*i == tamHeap)
			{
				filho = 2*i;
			}
			else{
			...
			if(array[i].getAltura().compareTo( array[filho].getAltura()) < 0)
			{
				Personagem tmp = new Personagem();
				tmp =
			}
	...
	public static void main (String[] Args)
	{
		...
		heap.construirHeap(linha,numEntrada);

		long startTime = System.nanoTime();

		//long endTime = System.nanoTime();
		//long execTime= (endTime - startTime);
		//Arq.openWriteClose("668007_sequencial.txt", "UTF-8", "Matricula: 668007\t"+"Tempo de execucao:\t"+execTime+"\t"+"Comparacoes:\t"+heap.comp);
	}
}
